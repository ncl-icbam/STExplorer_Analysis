---
title: "8PCW Retina analysis using STExplorer (SVGs)"
...
D1 <- readRDS("sfe_SVGs_D1.rds")

markers <- readRDS("markers.rds")
svgs <- readRDS("svgs.rds")
fgwc_list <- readRDS("fgwc_list_SVG.rds")
...
D1_8PCW_SVG_nmf_factors <- read_csv("D1_8PCW_SVG_nmf_factors.csv",  col_types = cols(...1 = col_skip()))
...
```{r}
D1_nmf_genes <- D1_8PCW_SVG_nmf_factors %>% filter(str_detect(external_gene_name, "^MT|^RPS|^RPL|^HB", negate = TRUE)) %>% arrange(desc(NMF2))
```
